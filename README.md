...
The detection layer is intentionally **defense-in-depth**: three independent telemetry pipelines — Suricata IDS/IPS at the network edge, Zeek NSM for protocol metadata, and Wazuh HIDS with Sysmon for endpoint events — each carry their own custom rules. Nine Suricata alert signatures, three Suricata IPS DROP signatures, four Zeek notice types, and six Wazuh rules with explicit MITRE ATT&CK mappings together cover every phase of the kill chain. Every layer fired during the simulation, every layer is mapped to a tactic in the coverage matrix, and the entire pipeline is unified inside a 10-visualization Wazuh OpenSearch dashboard for SOC-style triage.
...
Nine custom signatures cover the network-observable side of the kill chain — PowerShell User-Agent, `.ps1` URI, `/cmd` polling, `/exfil` POST, `/beacon` POST, `/result` POST, generic JSON POST to external, RDP-to-DC, and the `:33890` pivot port. Three additional DROP rules close the loop in IPS mode. The full ruleset lives in [`detection-rules/suricata/custom.rules`](detection-rules/suricata/custom.rules).
...
Representative rule (SID 1000001 — PowerShell User-Agent):
...
alert http $HOME_NET any -> $EXTERNAL_NET any ( \
  ...
  metadata:mitre_attack_id T1059.001, mitre_attack_tactic Execution, \
           signature_severity Major, deployment Perimeter, confidence High; \
  sid:1000001; rev:2;)
```
...
Six custom Wazuh rules in [`detection-rules/wazuh/local_rules.xml`](detection-rules/wazuh/local_rules.xml) target Sysmon-derived host events.
...
![Figure 44: Suricata alert counts — per-signature distribution from eve.json post-execution](images/fig44-suricata-alert-counts.png)

*Figure 44: Suricata alert counts — per-signature distribution from eve.json post-execution.*
...
![Figure 47: Wazuh custom rule 100001 details — hits with MITRE T1059.001 mapping on Gokuadm-mak agent](images/fig47-wazuh-rule-100001-details.png)

*Figure 47: Wazuh custom rule 100001 details — hits with MITRE T1059.001 mapping on Gokuadm-mak agent.*

![Figure 48: Wazuh custom rule 100001 details (continued) — full event context for analyst triage](images/fig48-wazuh-rule-100001-detail-2.png)

*Figure 48: Wazuh custom rule 100001 details (continued) — full event context for analyst triage.*
...
![Figure 50: RAT staging directory — wefs.cfg confirmed in C:\Users\Gokuadm-mak\AppData\Roaming\php](images/fig50-rat-staging-directory.png)

*Figure 50: RAT staging directory — wefs.cfg confirmed in C:\Users\Gokuadm-mak\AppData\Roaming\php.*
...
![Figure 52: C2 loot aggregate — beacon, exfil, and command-result JSON files from the full attack session](images/fig52-c2-loot-aggregate.png)

*Figure 52: C2 loot aggregate — beacon, exfil, and command-result JSON files from the full attack session.*
...
![Figure 53: Wazuh SIEM ingestion architecture — three telemetry streams unified into a single OpenSearch index](images/fig53-wazuh-siem-architecture.png)

*Figure 53: Wazuh SIEM ingestion architecture — three telemetry streams unified into a single OpenSearch index.*
